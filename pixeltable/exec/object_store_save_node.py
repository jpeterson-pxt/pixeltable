...
class ObjectStorePrefNode(ExecNode):
    ...
    in_flight_rows: dict[int, ObjectStorePrefNode.RowState]  # rows with in-flight urls; id(row) -> RowState
...
class ObjectStoreSaveNode(ExecNode):
    ...
    class WorkItem(NamedTuple):
        src_path: Path
        destination: str
        info: exprs.ColumnSlotIdx  # column info for the file being uploaded
        def pp(self) -> str:
    ...
    in_flight_rows: dict[int, ObjectStoreSaveNode.RowState]  # rows with in-flight urls; id(row) -> RowState
    in_flight_requests: dict[futures.Future, Any]  # in-flight requests for urls; future -> URL
    in_flight_urls: dict[WorkItem, list[tuple[exprs.DataRow, exprs.ColumnSlotIdx]]]  # URL -> [(row, info)]
    input_finished: bool
    row_idx: Iterator[Optional[int]]
    ...
    def __process_completions(self, done: set[futures.Future], ignore_errors: bool) -> None:
        for f in done:
            work_item = self.in_flight_requests.pop(f)
            new_file_url, exc = f.result()
            if exc is not None and not ignore_errors:
                raise exc

            # add the local path/exception to the slots that reference the url
            for row, info in self.in_flight_urls.pop(work_item):
                if exc is not None:
                    self.row_builder.set_exc(row, info.slot_idx, exc)
                else:
    ...
    def __process_input_batch(self, input_batch: DataRowBatch, executor: futures.ThreadPoolExecutor) -> None:
        ...
        for row in input_batch:
            ...
            for info in self.file_col_info:
                ...
                work_item = ObjectStoreSaveNode.WorkItem(src_path, '', info)
                for w in work_to_do:
                    if w.src_path == work_item.src_path and w.info.slot_idx == work_item.info.slot_idx:
                        # we already have this work item
                        continue
                self.in_flight_urls[work_item] = [(row, info)]
                num_missing += 1
                work_to_do.append(work_item)
        ...
        _logger.debug(f'submitting {len(work_to_do)} work items')
        for work_item in work_to_do:
            f = executor.submit(self.__move_tmp_data, work_item)
#            _logger.debug(f'submitted {work_item}')
            print(f'\n=========>> submitted {work_item.src_path} for column {work_item.info.col.name}\n')
            self.in_flight_requests[f] = work_item

    def __move_tmp_data(self, work_item: WorkItem) -> tuple[Optional[str], Optional[Exception]]:
        """Fetches a remote URL into the TempStore and returns its path"""

        src_path = work_item.src_path
        col = work_item.info.col
        _logger.debug(f'fetching src_path={src_path} thread_name={threading.current_thread().name}')
        try:
            new_file_url = MediaStore.relocate_local_media_file(src_path, col)
            _logger.debug(f'moved {src_path} to {new_file_url}')
            return new_file_url, None
        except Exception as e:
            _logger.debug(f'Failed to move {src_path}: {e}', exc_info=e)
            return None, e
