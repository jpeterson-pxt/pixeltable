...
class Planner:
    ...
    def _insert_prefetch_node(
        ...
        prefetch_node = exec.ObjectStorePrefNode(tbl_id, file_col_info, input_node)
    ...
    def create_add_column_plan(cls, tbl: catalog.TableVersionPath, col: catalog.Column) -> exec.ExecNode:
        ...
        if col.is_computed and col.is_stored and col.col_type.is_image_type():
            stored_img_cols = row_builder.output_slot_idxs()
        if col.is_computed and col.is_stored and col.col_type.is_media_type():
            stored_media_cols = row_builder.output_slot_idxs()  

        # we need to add the column to the row builder
        plan = cls._insert_save_node(tbl.tbl_version.id, stored_media_cols, input_node=plan)

        # we want to flush images
        if len(stored_img_cols) > 0:
            plan.set_stored_img_cols(stored_img_cols)
        return plan
